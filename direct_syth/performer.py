...
class FastAttention(nn.Module):
    def __init__(self, dim_heads, nb_features = None, feature_redraw_interval = 1000, ortho_scaling = 0, #causal = False, 
...
class SelfAttention(nn.Module):
    ...
    heads = 8, local_heads = 0, local_window_size = 256, nb_features = None, 
    feature_redraw_interval = 1000, generalized_attention = False, 
    kernel_fn = nn.ReLU(), qr_uniform_q = False, dropout = 0.):
        super().__init__()
        assert dim % heads == 0, 'dimension must be divisible by number of heads'
        dim_head = dim // heads
        self.fast_attention = FastAttention(dim_head, nb_features, feature_redraw_interval, #causal = causal, 
                generalized_attention = generalized_attention, kernel_fn = kernel_fn, qr_uniform_q = qr_uniform_q)

        self.heads = heads
        self.global_heads = heads - local_heads
        causal = False
        self.local_attn = LocalAttention(window_size = local_window_size, 
                            autopad = True, dropout = dropout, look_forward = int(not causal), 
